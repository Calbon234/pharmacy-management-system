...
const styles = `
  ...
  .stat-card { background: #fff; border-radius: 16px; padding: 22px; border: 1px solid #e8eef5; position: relative; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; animation: fadeUp 0.4s ease both; box-shadow: 0 1px 6px rgba(0,0,0,0.04); cursor: pointer; }
...
export default function Dashboard() {
  ...
  const kpis = stats ? [
    { cls: "c1", icon: "💊", val: stats.total_medicines.toLocaleString(), lbl: "Drugs in Stock", chip: `${stats.low_stock} low`, chipCls: stats.low_stock > 0 ? "warn" : "up", path: "/inventory" },
    { cls: "c2", icon: "📋", val: stats.pending_rx, lbl: "Pending Prescriptions", chip: "Pending", chipCls: "warn", path: "/prescriptions" },
    { cls: "c3", icon: "💰", val: `KES ${stats.today_sales.toLocaleString()}`, lbl: "Today's Revenue", chip: `${stats.today_transactions} sales`, chipCls: "up", path: "/sales-history" },
      ...
      <div className="dash">
        ...
        <div className="stat-grid">
          {loading ? [1,2,3,4].map(i => (
            ...
            <div key={i} className={`stat-card ${s.cls}`} onClick={() => navigate(s.path)}>
              ...
              <table className="mini-tbl">
                ...
                <tbody>
                  {stats?.low_stock_list?.length ? stats.low_stock_list.map((d, i) => {
                    ...
                    return (
                      <tr key={i} style={{cursor:"pointer"}} onClick={() => navigate("/inventory")}>
          ...
          <div className="panel">
            ...
            {loading ? (
              ...
              <div style={{fontSize:13,color:"#374151",padding:"12px 0"}}>
                <div onClick={() => navigate("/expired")} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"#fef3c7",borderRadius:10,marginBottom:8,cursor:"pointer"}}>
                  <span style={{fontSize:20}}>⚠️</span>
                  <div>
                    <div style={{fontWeight:700,color:"#b45309"}}>{stats.expiring_soon} medicines expiring within 30 days</div>
                    <div style={{fontSize:12,color:"#92400e",marginTop:2}}>Click to view details</div>
                  </div>
                </div>
                {stats.expired > 0 && (
                  <div onClick={() => navigate("/expired")} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:"#fee2e2",borderRadius:10,cursor:"pointer"}}>
                    <span style={{fontSize:20}}>🚨</span>
                    <div>
                      <div style={{fontWeight:700,color:"#dc2626"}}>{stats.expired} medicines already expired!</div>
                      <div style={{fontSize:12,color:"#b91c1c",marginTop:2}}>Click to remove from shelves</div>
                    </div>
                  </div>
                )}
              </div>
