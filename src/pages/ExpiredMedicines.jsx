...
export default function ExpiredMedicines() {
  ...
  const getDaysLeft = (expiry) => {
    ...
    const diff = Math.ceil((new Date(expiry) - today) / (1000 * 60 * 60 * 24));
    return diff;
  };
  ...
  const filtered = medicines
    .map(m => ({ ...m, daysLeft: getDaysLeft(m.expiry_date), status: getStatus(getDaysLeft(m.expiry_date)) }))
    .filter(m => {
      if (tab === "expired") return m.status === "expired";
      if (tab === "expiring") return m.status === "expiring";
      if (tab === "all") return m.status === "expired" || m.status === "expiring";
      return true;
    })
    .filter(m => m.name.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => a.daysLeft - b.daysLeft);

  const expiredCount  = medicines.filter(m => getStatus(getDaysLeft(m.expiry_date)) === "expired").length;
  const expiringCount = medicines.filter(m => getStatus(getDaysLeft(m.expiry_date)) === "expiring").length;
  ...
  return (
    <div style={S.pg}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 20 }}>
        {[
          { icon: "🚨", label: "Expired", val: expiredCount, color: "#f43f5e", bg: "rgba(244,63,94,0.1)" },
          { icon: "⚠️", label: "Expiring Soon", val: expiringCount, color: "#f59e0b", bg: "rgba(245,158,11,0.1)" },
          { icon: "💊", label: "Total Monitored", val: medicines.length, color: "#6366f1", bg: "rgba(99,102,241,0.1)" },
          { icon: "✅", label: "Safe Stock", val: medicines.filter(m => getStatus(getDaysLeft(m.expiry_date)) === "ok").length, color: "#00e5c0", bg: "rgba(0,229,192,0.1)" },
        ].map((k, i) => (
      ...
      {!loading && expiredCount > 0 && (
        <div style={{ background: "#fee2e2", border: "1px solid #fecaca", borderRadius: 12, padding: "14px 18px", marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "#b91c1c", marginTop: 2 }}>Remove these from shelves immediately to avoid dispensing errors.</div>
          </div>
        </div>
      )}
              ...
              {loading ? [1, 2, 3].map(i => (
                ...
                <tr>
                  <td colSpan={7} style={{ textAlign: "center", padding: 48, color: "#8a9ab5" }}>
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700 }}>No expired or expiring medicines</div>
                    <div style={{ fontSize: 12, marginTop: 4 }}>All stock is within safe expiry dates</div>
                  </td>
                </tr>
              ) : filtered.map((m, i) => {
                const isExpired  = m.status === "expired";
                const isExpiring = m.status === "expiring";
                const tagBg    = isExpired ? "#fee2e2" : "#fef3c7";
                const tagColor = isExpired ? "#dc2626" : "#b45309";
                const tagLabel = isExpired ? "Expired" : "Expiring Soon";
                ...
                return (
                  <tr key={i} style={{ borderTop: "1px solid #f0f4f8", background: isExpired ? "rgba(254,226,226,0.3)" : isExpiring ? "rgba(254,243,199,0.3)" : "transparent" }}
                    ...
                    <td style={{ padding: "13px 16px", fontSize: 13, fontWeight: 700, color: isExpired ? "#dc2626" : "#b45309" }}>{daysText}</td>
                    <td style={{ padding: "13px 16px" }}>
